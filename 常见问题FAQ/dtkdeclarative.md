---
title: dtkdeclarative——DTK开发者工具套件
description: 
published: true
date: 2022-10-18T07:46:47.075Z
tags: 
editor: markdown
dateCreated: 2022-10-18T07:46:47.075Z
---

# DTK（developer toolkit）开发者工具套件
## dtkdeclarative的基础和创新
...
## dtkdeclarative 和 dtkwidget 的不同
...
dtkwidget 和 dtkdeclarative 在渲染方式上的区别
...
## dtkwidget 和 dtkdeclarative 在布局方式上的区别
...
dtkwidget 和 dtkdeclarative 在信号处理上的区别
...
### DCI （DSG combined icons，DSG混合图标）

DCI图标的实现来源于需求开发问题，用户界面需要展示更多状态图标，现有图标资源加载引擎技术效率不高，为解决对应技术问题，DCI图标技术应运而生。其原理结构如下：

DCI图标一种归档打包格式（后缀名 .dci ），格式名称为 dci (DSG combined icons，DSG1混合图标)，mimetype类型为 image/dci ，继承于 application/octet-stream 。magic值为 DCI ，类型为 string ，offset 为 0 。此类型对应的图标名： image-dci 。相应DCI资源路径示例如下：

基于DCI图标技术，用户界面开发能够满足多种交互状态的响应，能够满足多种图标类型的使用，能够满足多种DPI缩放规格的适应，在图标管理上更灵活，在需求实现上更优雅。
ColorSelector，颜色选择器/颜色控制器
...
### ColorSelector 
  ...
  由调色板、控件属性和取色器三个层次组成。其原理结构和代码示例如下：
...
原有在 dtkwidget 下开发的 SettingDialog 使用 Json 文件声明布局和控件，这种方式对于布局实现存在一定局限性，不适用高级组合控件，会造成显示重叠、错乱等问题，在代码维护方面更是牵一发而动全身。随着业务和需求的变化，解决融合 QML 开发的问题，借此时机摒弃 Json ，使用 dtkdeclarative 树状视图代码风格，提供一套可维护性和易读性俱佳的统一控件模板。如下即是全新 SettingDialog 的示例代码：
...
为了更高效地对接 Config配置方案2，将 SettingDialog 中的设置选项与 Config 中的配置选项打通，符合qml语法且使用 dtkdeclarative 开发的全新Settings框架由此诞生，实现“一处声明，全局可用”。
