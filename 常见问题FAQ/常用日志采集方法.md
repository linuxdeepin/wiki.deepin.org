...
# 系统崩溃、卡死、死机
可以尝试TTY（ctrl+alt+F2）是否能正常切换，键盘大小写灯能否正常显示，若以上都无反应，则可强制重启机器并提取内核日志  ```cat ~/kern.log | grep -i "Call Trace"``` 查看是否存在内核模块崩溃的情况, 更换内核版本再次查看是否死机，并提供日志到研发分析。
...
```
**还可能需要辅助coredump日志**
[参考此篇wiki学习使用coredump工具](https://wiki.deepin.org/zh/Coredump工具使用与日志导出)
# 无法进入系统
**当遇到类似如下问题时：**
- 开机无法进入系统
- 终端锁屏后无法登录系统
...
```
**可能需要辅助coredump日志**
[参考此篇wiki学习使用coredump工具](https://wiki.deepin.org/zh/%E5%B8%B8%E8%A7%81%E9%97%AE%E9%A2%98FAQ/Coredump%E5%B7%A5%E5%85%B7%E4%BD%BF%E7%94%A8%E4%B8%8E%E6%97%A5%E5%BF%97%E5%AF%BC%E5%87%BA)
# 系统常见模块日志路径
# DDE
